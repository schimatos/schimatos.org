...
\ta sh:NodeShape ;${targetByPrefixes(state)}` + shaclFile
    ...
    const initialState = {...base, turtle : remakeTtl(base)}

    const targOpts = (state) => ['Node', 'Class', 'ObjectsOf', 'SubjectsOf'].map(n => {
        return [IRI, n, {placeholder : `Target ${n}${n === 'Class' ? 'e' : ''}s`, additionLabel : `Add ${n}: `, options : state[n], search :true,  noResultsMessage : `Type the name of the ${n} you wish to associate with this shacl.`, multiple : true, fluid : true, selection : true, allowAdditions : true}, `${n}${n === 'Class' ? 'e' : ''}s`,{routine : x => ({...x, turtle :  remakeTtl(x)})}]
    })
    const content = state => [
        ['Shacl Name', [Name, 'name', {placeholder : 'Name...'},,{routine : x => ({...x, turtle :  remakeTtl(x)})}]],
        ['Shacl Targets', ...targOpts(state)],
        ['Shacl File', [props => <TextArea {...{...props, style : {width : '100%'}}} />, 'turtle',,,{routine : x => {
            console.log('routine', x)
            const newState = {...x, turtle :  remakeTtl(x)}
