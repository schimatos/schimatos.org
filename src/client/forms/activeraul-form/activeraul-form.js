...
import {Segment, Grid, Dropdown, Popup, Input, Button, Icon, Confirm} from 'semantic-ui-react'
...
import {keepCloning, hashDict, fromCamel, camelToSnake, contentReduce, extendDict, extendArray, hyperlink, eqHash, hash} from '../../utils'
...
export default () => {
   ...
   setTimeout(() => {
    ...
    const PropertySegment = ({children, id, pathOpen}) => {
        ...
        const {path, pathType, severity, maxCount, message, minCount, name, description, class:cl} = propertyList[property]
        ...
        const header = {
            ...
            title : {
                content : pathType === ('alternativePath' ? (
        ...
        const TargetDropdown = (i, value, c) => {
            ...
            const {field, check} = targets.renderedValidators[id] ? targets.renderedValidators[id]({value, onSubmit : (v, t) => addOption(i, id, v, t), name : `${i}`, options : opts, makeIRI, displayIRI}) : {field : <div></div>, check : false}
            //const Field = field
            return (
                ...
                <DynamicButton
                    ...
                    name='x'
                    attached='right'
                    disabled={(atMin && violation) || c.length > 0}
                    giveWarning={atMin}
                    floated={'right'}
