...
export const MultiWordField = (props) => {
    ...
    const RegexedField = ({ fields, values, registrations, onSubmit, remakeWord, value, name, emptyFields, pixelsTaken, primaryValidation }) => {
                ...
                <Grid.Row centered style={{ padding: '0px', margin: '0px' }}>
                    {fields.map((x, i) => <Grid.Column key={i} verticalAlign={'bottom'} style={typeof (x) === 'string' ? { margin: '0px', padding: '0px', width : `auto` } : { width: x.maxchar ? `${(x.maxchar + 4)*12}px` : `${(width-pixelsTaken-12)/emptyFields}px`, padding: '3px', margin : '0px' }}>
                        ...
                        {typeof (x) !== 'string' ? 
                            <Form.Input {...x}
                                fluid
                                style={{ width: x.maxchar ? `${(x.maxchar + 3.5)*12}px` : 'auto', margin: '0px', padding :" 0px" }}
                                error={errors[`${name}w${i}`] ? { content: errors[`${name}w${i}`].message, pointing: 'below' } : (error[name] ? (i=== 0 ? { content: errors[name].message, pointing: 'below' } : true) : false)}
