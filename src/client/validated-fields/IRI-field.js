...
export default base_graph => {
    ...
    const unpackSingle = iri => {
        const [k, v] = valuesMatch(prefixes, v => startMatch(v, iri), v => v.length)
        return [k, iri.slice(v.length,)]
    }
    ...
    return ({value, graph, validator, ...other}) => {
       ...
       // const iri_regex = new RegExp(String.raw`^(([^:/?#]+):)?(//([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))?`, 'i')
        ...
        const {onClose, onFocus, onChange, onKeyPress, isValidating, defaultValue, error} = useFormCustom({...other, validator, unpack, repack, value : v})
