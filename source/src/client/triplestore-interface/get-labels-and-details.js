...
export default async (props) => {
    ...
    const r = await [...await response_labels, ...await response_labels2,  ...await response_labels3]
    // console.log(await r)
    const r2 = r.reduce((t, x) => ({...t, [x.iri.value] : x.value.value}), {})
    // console.log(await r2)


    const fullDetails = IRIS.length > 0 ? await requester(`SELECT DISTINCT * FROM {knamed} WHERE {?iri ?p ?value VALUES(?iri) {${IRIS.reduce((t,x) => t + `(<${x}>)`,'')}}}`, 'kgraph', triplestore, x => x, {}) :[]
    // console.log(await fullDetails)
    const fullDetailsReduced = fullDetails.reduce((t, x) => ({...t, [x.iri.value] : {
        ...t.iri.value,
        [x.p.value] : t.p.value ? [...t.p.value, x.value.value] : [x.value.value]
    }
    }), {})

    return await [r2, fullDetailsReduced]
}