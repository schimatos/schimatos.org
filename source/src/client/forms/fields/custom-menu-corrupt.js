...
import FieldModal from './field-modal-corrupt'
...
export default ({className, fixed, warnings, popups, icons}) => {
    ...
    const reduceDetails = (ic, d, idn) => ic.reduce(([i, w, ix], icon) => {
        ...
        const {initialState, content, header, conditions, advanced, confirmation} = modal ? modal : {}
        ...
        const newWarns = {...w, [ix] : {warning, onClick, initialState, content, header, conditions, warnClick, advanced, confirmation}}
    ...
    const menuItem = pos => (item, key) => {
        ...
        const contenta = <Menu.Item key={`${pos}${key}`} as={link && Segment} basic={link&&true} active={activated} disabled={disabled} onClick={click}>{icon&&<Icon key={`ic${pos}${key}`} name={icon} inverted={selected}/>}{image&&<img key={`img${pos}${key}`} style={{width : '25px', height : '22px'}} src={`../../../../public/${image}`} alt={(!icon && !text) ? image.split('.')[0] : ''}/>}{text}</Menu.Item>
        const content = link ? <a key={`link${pos}${key}`} href={link} target="_blank">{contenta}</a> : contenta
        return (popup && popups !== false) ? <Popup key={`pop${pos}${key}`} content={popup} position={`bottom ${pos}`} trigger={content} hoverable={typeof(popup) !== 'string'} on={'hover'}/> : content
    }
    ...
    const menu = (
        <Menu className={className} attached={fixed} style={{margin : '0px'}}>
