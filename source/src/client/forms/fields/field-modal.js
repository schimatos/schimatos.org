...
export default ({submit, open, close, initialState, content, header, conditions, advanced, submitButton, confirmation, defaultActiveIndex}) => {
    ...
    const onClick = () => {
        //console.log('state at on Click', state)
        return confirmation ? setWarning(true) : submit(state)
    }
    const onConfirm = () => {
        setWarning(false)
        submit(state)
    ...
    return (
        ...
        <Modal.Content><Accordion exclusive={false} fluid defaultActiveIndex={defaultActiveIndex || []} panels={panels} styled/></Modal.Content>
            <Modal.Actions>
