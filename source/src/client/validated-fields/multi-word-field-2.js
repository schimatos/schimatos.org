...
export const MultiWordField = (props) => {
    ...
    const RegexedField = ({ fields, values, registrations, onSubmit, remakeWord, value, name, emptyFields, pixelsTaken, primaryValidation }) => {
        ...
        const submit = e => {
            ...
            setValue(name, remakeWord(getValues()))
                    ...
                    {fields.map((x, i) => <Grid.Column relaxed={false} key={i} verticalAlign={'bottom'} style={typeof (x) === 'string' ? { margin: '0px', padding: '0px'} : {padding: '1px', margin : '0px', width:'175px' }}>
                        ...
                        {typeof (x) !== 'string' ? 
                            <Form.Input {...x}
                                ...
                                error={errors[`${name}w${i}`] ? { content: errors[`${name}w${i}`].message, pointing: 'below' } : (errors[name] ? (i=== 0 ? { content: errors[name].message, pointing: 'below' } : true) : false)}
