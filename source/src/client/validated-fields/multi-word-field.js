...
export const MultiWordField = (props) => {
    ...
    const RegexedField = ({ fields, values, registrations, onSubmit, remakeWord, value, name, emptyFields, pixelsTaken }) => {
                    ...
                    {fields.map((x, i) => <Grid.Column key={i} verticalAlign={'bottom'} style={typeof (x) === 'string' ? { margin: '0px', padding: '0px', width : `auto` } : { width: x.maxchar ? `${(x.maxchar + 4)*12}px` : `${(width-pixelsTaken-12)/emptyFields}px`, padding: '3px', margin : '0px' }}>
                        ...
                        {typeof (x) !== 'string' ? 
                            <Form.Input {...x}
                                ...
                                error={errors[`${name}w${i}`] ? { content: errors[`${name}w${i}`].message, pointing: 'below' } : false}
    ...
    const getDetails = (props, state) => {
        ...
        const registrations = fields.reduce((total, field, i) => {
            if (field[1]) {
                const toTest =/^[0-9]{2}\/[0-9]{2}\/[0-9]{4}$/  // makeRegexp('/'+field[0]+'/') makeRegexp(field[0])
